...
def get_notification_list(user):
    ...
    def build_resolution_message(res):
        return (
            ...
            % (res.other_user, res.relative_value_repr),
...
def settings(request):
    ...
    context["user_form"] = forms.UserEditForm(
        form_data(request), instance=request.user
    )
    context["password_form"] = forms.PasswordChangeForm(form_data(request))
    ...
    context["exchange_rate_form"] = forms.ExchangeRateForm(form_data(request))
...
def transaction_list(request):
    ...
    return render(request, "transaction_list.html", context=context)
...
def transaction_modify(request, trans_record_id):
    """Modify a transaction record from another user."""
