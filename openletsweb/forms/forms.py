...
from openletsweb.forms.base import *
User = get_user_model()
__all__ = [
...
class TransactionRecordForm(BaseFormMixin, forms.ModelForm):
    ...
    def save(self, active_user, commit=True):
        """Save the model. Make sure users are assigned to correct
        field.
        """
        trans_rec = super(TransactionRecordForm, self).save(commit=False)
        data = self.cleaned_data

        trans_rec.creator_user = active_user
        # TODO: cleaner error message
        assert trans_rec.target_user != active_user
...
class TransferListForm(BaseFormMixin, forms.Form):
    ...
    _parts = {
        ...
        "left_col": ["user", "currency", "transaction_time", "confirmed_time"],
    }
...
class ExchangeRateForm(BaseFormMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ("source_currency", "source_rate", "dest_currency", "dest_rate")
    ...
    def clean(self):
        ...
        source_value, source_error = currency_clean_helper(
            cleaned_data["source_currency"], cleaned_data["source_rate"]
        )
        dest_value, dest_error = currency_clean_helper(
            cleaned_data["dest_currency"], cleaned_data["dest_rate"]
        )
        if source_error:
