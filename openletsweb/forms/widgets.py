...
class RadioButton(core_widgets.RadioSelect):
    ...
    def __unicode__(self):
        ...
        return mark_safe(
            "<li><label>%s<span> %s</span></label></li>" % (self.tag(), choice_label)
        )
...
#             % ("\n".join("%s" % force_str(w) for w in self))
#         )
...
class JUISlider(core_widgets.TextInput):
    ...
    default_attrs = {"class": "slider-input small", "readonly": "readonly"}
    ...
    def render(self, name, value, attrs=None):
        ...
        if value:
            attrs["value"] = force_str(self._format_value(value))

        return mark_safe(
            "<span class='slider-label'>Include days:</span>"
            "<input %s/>" % attrs + "<div class='slider'></div>"
        )
