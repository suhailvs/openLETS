...
def find_similar(users_pending_trans, other_user_trans_record):
    ...
    def user_match(trans_record):
        return trans_record.creator_user == other_user_trans_record.target_user
    ...
    candidates = filter(user_match, users_pending_trans)
    print(len(list(candidates)))
    if not len(list(candidates)):
        return None
