...
currency_field = lambda **kwargs: m.ForeignKey(
    "Currency", on_delete=m.CASCADE, related_name="+", **kwargs
)
...
class Balance(CurrencyMixin, m.Model):
    ...
    persons = m.ManyToManyField(
        "Person", through="PersonBalance", blank=True, related_name="balances"
    )
...
class PersonBalance(m.Model):
    ...
    person = m.ForeignKey("Person", on_delete=m.CASCADE)
...
class TransactionRecord(DictableModel, CurrencyMixin, m.Model):
    ...
    creator_person = m.ForeignKey(
        "Person", on_delete=m.CASCADE, related_name="transaction_records_creator"
    )
...
class Resolution(CurrencyMixin, m.Model):
    ...
    persons = m.ManyToManyField(
        "Person", through="PersonResolution", blank=True, related_name="resolutions"
    )
...
class PersonResolution(m.Model):
    ...
    person = m.ForeignKey("Person", on_delete=m.CASCADE)
    resolution = m.ForeignKey("Resolution", on_delete=m.CASCADE)
    credited = m.BooleanField()

    def __unicode__(self):
        return "PersonResolution for %s and %s" % (self.person, self.resolution)
